...
-- определяем типы лайков через boolean, т.е. like = 1, dislike = 0
...
CREATE TABLE likes (
id SERIAL PRIMARY KEY,
user_id_from BIGINT UNSIGNED NOT NULL COMMENT "кто лайкнул",
user_id_to BIGINT UNSIGNED NOT NULL COMMENT "кому лайкнул",
media_id BIGINT UNSIGNED NOT NULL COMMENT "что лайкнул",
like_type BIGINT UNSIGNED NOT NULL COMMENT "как лайкнул",
created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT "когда лайкнул",
KEY (user_id_from),
KEY (user_id_to),
KEY (media_id),
KEY (like_type),
CONSTRAINT fk_who_liked FOREIGN KEY (user_id_from) REFERENCES users (id),
CONSTRAINT fk_whom_liked FOREIGN KEY (user_id_to) REFERENCES users (id),
CONSTRAINT fk_media_being_liked FOREIGN KEY (media_id) REFERENCES media (id),
CONSTRAINT fk_how_liked FOREIGN KEY (like_type) REFERENCES liketypes(id)
);
